...
export async function getTradesForAccount(accountId?: string): Promise<AliceTrade[]> {
  ...
  // If OAuth token exists, use OAuth endpoint (Alice Blue API)
  if (token) {
    const oauthEndpoint = process.env.ALICE_OAUTH_TRADES_ENDPOINT || 'https://ant.aliceblueonline.com/open-api/od/v1/trades';
    try {
      const headers = {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`,
      };
      console.log(`[ALICE] Fetching trades for ${accountId} via OAuth endpoint: ${oauthEndpoint}`);
      const res = await fetchWithRetry(oauthEndpoint, { headers });
      const payload = await res.json().catch(() => ({}));
      const source = payload.trades || payload.data || payload || [];
      
      console.log(`[ALICE] OAuth fetch for ${accountId}: received ${Array.isArray(source) ? source.length : 0} trades`);
      ...
      console.error(`[ALICE] OAuth endpoint failed for ${accountId}:`, e);
      // Continue to API key auth fallback
    }
  }
